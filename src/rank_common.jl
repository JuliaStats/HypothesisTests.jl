...
# selection of the exact/approximate method, guards on the exact null
# distributions, the contrast sets the distribution-free intervals are read off,
# and the two index rules used to invert the test.
...
## GUARDS ON THE EXACT NULL DISTRIBUTIONS
#
# `StatsFuns.signrankcdf` and `StatsFuns.wilcoxcdf` accumulate lattice counts in
# `Int` and are silently invalid once those counts overflow. The two bounds below
# were found by comparing against `Rmath.psignrank`/`Rmath.pwilcox` over the whole
# support: both are sharp, and validity is monotone on either side of them.
#
# See JuliaStats/StatsFuns.jl#219 (signrank) and #220 (wilcox).

"Largest number of non-zero observations for which `signrankcdf` is valid."
const MAX_EXACT_SIGNRANK_N = 71

"Largest `binomial(nx + ny, nx)` for which `wilcoxcdf` is valid."
const MAX_EXACT_WILCOX_BINOMIAL = 3_009_106_305_270_645_216

function check_exact_signrank(n::Integer)
    n <= MAX_EXACT_SIGNRANK_N && return nothing
    throw(ArgumentError(
        "the exact signed rank null distribution is unavailable for $n non-zero " *
        "observations: `StatsFuns.signrankcdf` silently returns invalid values for " *
        "n > $MAX_EXACT_SIGNRANK_N (JuliaStats/StatsFuns.jl#219). " *
...
function check_exact_wilcox(nx::Integer, ny::Integer)
    count = rank_binomial(nx, ny)
    (count !== nothing && count <= MAX_EXACT_WILCOX_BINOMIAL) && return nothing
    throw(ArgumentError(
        "the exact Mann-Whitney null distribution is unavailable for nx = $nx, ny = $ny: " *
        "`StatsFuns.wilcoxcdf` silently returns invalid values once " *
        "binomial(nx + ny, nx) exceeds $MAX_EXACT_WILCOX_BINOMIAL " *
        "(JuliaStats/StatsFuns.jl#220). Pass `method = :approximate`."))
end
...
rank sample sizes at `level = 0.95` and 44 of 66 at `level = 0.90`; with it the interval
...
[`confint`](@ref) is built around, and the quantity R's `wilcox.test` reports as
`estimate`. It is generally not the sample median.
"""
