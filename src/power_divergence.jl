...
function PowerDivergenceTest{T<:Integer,U<:AbstractFloat}(x::AbstractMatrix{T}; lambda::U=1.0, theta0::Vector{U} = ones(length(x))/length(x))
    ...
    #validate data
    any(x .< 0) && error("all entries must be nonnegative and finite")
    n == 0 && error("at least one entry must be positive")
    # TODO: Is there any reason an AbstractMatrix could have infinite dimensions?
    if !(isfinite(nrows) && isfinite(ncols) && isfinite(nrows*ncols))
    ...
    if nrows > 1 && ncols > 1
        ...
        theta0 = (xhat/n)[:]
        V = Float64[@compat(Float64(colsums[j]))*rowsums[i]*(n-rowsums[i])*(n-colsums[j]) /
                    @compat(Float64(n))^3
    ...
    elseif nrows == 1 || ncols == 1
        ...
        abs(1 - sum(theta0)) <= sqrt(eps()) || error("Probabilities must sum to one")
    else
