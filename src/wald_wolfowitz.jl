...
function StatsAPI.pvalue(test::WaldWolfowitzTest; tail=:both)
    return pvalue(Normal(0.0, 1.0), test.z; tail=tail)
end

function show_params(io::IO, x::WaldWolfowitzTest, ident="")
    println(io, ident, "number of runs:  $(x.nruns)")
    return println(io, ident, "z-statistic:     $(x.z)")
end
...
function WaldWolfowitzTest(x::AbstractVector{Bool})
    ...
    z = (nruns - μ) / σ
    return WaldWolfowitzTest(nabove, nbelow, nruns, μ, σ, z)
end
