...
@testset "Power Divergence" begin
    ...
    @test m.theta0 ≈ [0.25523082406125785, 0.19670969099133556, 0.11593952361049113,
                      0.08935608756107216, 0.1935739395970214, 0.1491899341788219]
    @test m.thetahat ≈
          [0.2763873775843308, 0.1755531374682626, 0.11860718171926006, 0.08668842945230323,
           0.16974972796517954, 0.17301414581066377]

    c = confint(m, method=:sison_glaz)
    c0 = [(0.25788900979, 0.29554669435),
          (0.15705476968, 0.19471245423),
          (0.10010881393, 0.13776649848),
          (0.06819006166, 0.10584774622),
          (0.15125136017, 0.18890904473),
          (0.15451577802, 0.19217346257)]
    ...
    @test m.df ≈ 2
    @test m.n ≈ 2757
    @test m.residuals ≈ reshape([2.198855766015898, -2.504669492560728, 0.4113701700566286,
                                 -0.46858294710127296, -2.8432397155451494, 3.2386734435365825], size(d))
    @test m.stdresiduals ≈
          reshape([4.502053521086705, -4.502053521086705, 0.6994517329844298,
                   -0.6994517329844298, -5.315945542704929, 5.315945542704929], size(d))
    ...
    c = confint(m)
    c0 = [(0.21666666667, 0.48098082062),
          (0.13333333333, 0.39764748728),
          (0.30000000000, 0.56431415395)]
    ...
    @test m.n == 60
    @test m.residuals ≈
          reshape([-1.5811388300841895, -0.6324555320336759, -0.31622776601683794,
                   -0.6324555320336759, 0.0, 3.162277660168379], size(O))
