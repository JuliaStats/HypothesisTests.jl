...
@testset "Permutation" begin
    ...
    @testset "ApproximatePermutationTest" begin
        rng = StableRNG(12345)
        @test pvalue(ApproximatePermutationTest(rng, [1, 2, 3], [4, 5, 6], mean, 100),
                     tail=:both) ≈ 0.14
        @test pvalue(ApproximatePermutationTest(rng, [4, 5, 6], [1, 2, 3], mean, 100),
                     tail=:both) ≈ 0.08
        @test pvalue(ApproximatePermutationTest(rng, [1, 2, 3], [4, 5, 6], mean, 100),
                     tail=:left) ≈ 0.05
        @test pvalue(ApproximatePermutationTest(rng, [4, 5, 6], [1, 2, 3], mean, 100),
        ...
        @test ApproximatePermutationTest([4, 5, 6], [1, 2, 3], mean, 100) isa
              HypothesisTests.PermutationTest{Float64}
    end
end
