...
@testset "Anderson-Darling" begin
    ...
    @testset "k-sample test" begin
        samples = Any[[38.7, 41.5, 43.8, 44.5, 45.5, 46.0, 47.7, 58.0],
    ...
    @testset "more tests" begin
        ...
        samples = Any[rand(rng, Normal(), 50), rand(rng, Normal(), 30),
                      rand(rng, Normal(), 20)]
        t = KSampleADTest(samples...)
        @test pvalue(t) > 0.05

        @test pvalue(OneSampleADTest(vcat(rand(rng, Normal(), 500),
                                          rand(rng, Beta(2, 2), 500)), Beta(2, 2))) ≈ 0.0 atol=0.1^4
