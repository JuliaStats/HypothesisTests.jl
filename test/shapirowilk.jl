...
if VERSION >= v"1.8"
    # to make sure rand is stable
    Random.seed!(1)
    X = sort(rand(Float64, 10))
    # W, pval, _ = swilkfort(X)
    W = 0.8396421171963104
    pval = 0.043690762992353756

    @test HypothesisTests.swstat(X, SWc10) ≈ 0.8396421171963104 atol=eps(Float32)
    @test pvalue(HypothesisTests.swstat(X, SWc10), SWc10) ≈ 0.043690762992353756 atol=eps(Float32)
end
